...
const BASE_URL_AI = "https://luckycoin.im"
const BASE_URL = "https://data-alpha.sending.me"

const storeMiddleware =
  (next) => (key: string, _nextFetcher: unknown, config: unknown) =>
    next(
      key,
...
// api
const apiMiddleware =
  (next) => (key: string, nextFetcher: unknown, config: unknown) => {
    if (typeof key === "string" && key.startsWith("api:/")) {
      return next(
        key,
        async (_key: string, payload: unknown) => {
          const apiAccessToken = await storageLoad("apiAccessToken")
          let url = BASE_URL + "/dex-api/v1/data" + key.substring(4)
          let headers = {}
          if (key.startsWith("api:/ai/")) {
            url = BASE_URL_AI + "/dex-api/client/r0" + key.substring(7)
          } else {
            headers = {
              "X-API-KEY": "41DD2ABE-41B0-201A-A25A-F4E54B50A71B",
            }
          }
          const fetchConfig = {
            ...(payload?.arg ?? {}),
